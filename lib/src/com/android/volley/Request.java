...
public abstract class Request<T> implements Comparable<com.android.volley.Request<T>> {
    ...
    public String getUrl() {
        ...
        if (this.mMethod == Method.GET) {
            ...
            if (params != null && params.size() > 0) {
                String param = encodeParameterStr(params,
                        getParamsEncoding());
                SLog.w(">>>>>>>>param",param);
                int len = ret.indexOf("?");
                if (len > 0) {
                    ret = HString.concatString(ret, "&", param);
                } else {
                    ret = HString.concatString(ret, "?", param);
                }
            }
    ...
    static Pattern p=Pattern.compile("\\[(\\d{1,2})\\]$");
    /**
     * Converts <code>params</code> into an application/x-www-form-urlencoded
     * encoded string.
     */
    protected String encodeParameterStr(Map<String, String> params,
                                        String paramsEncoding) {
        SLog.w(">>>>>>>>>>",paramsEncoding);
        StringBuilder encodedParams = new StringBuilder();
        try {
            for (Map.Entry<String, String> entry : params.entrySet()) {
                String k=entry.getKey();
                final Matcher m = p.matcher(k);

                if(m.find()){
                    k=k.subSequence(0,m.start())+"[]";
                }
                SLog.w("set http param:", k);
                encodedParams.append(URLEncoder.encode(k,
                        paramsEncoding));
